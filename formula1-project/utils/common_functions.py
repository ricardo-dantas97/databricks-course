...
def merge_delta_data(input_df, db_name, tbl_name, folder_path, merge_condition, partition_column):
    spark.conf.set('spark.databricks.optimizer.dynamicPartitionPruning', 'true')

    from delta.tables import DeltaTable
    if (spark._jsparkSession.catalog().tableExists(f'{db_name}.{tbl_name}')):
        deltaTable = DeltaTable.forPath(spark, f'{folder_path}/{tbl_name}')
        ...
        df.write.mode('overwrite').partitionBy(partition_column).format('delta').saveAsTable(f'{db_name}.{tbl_name}')
